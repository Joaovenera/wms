...
import UcpItemsViewer from "@/components/ucp-items-viewer";

// Use the Ucp type from api.ts directly since it already has the relations
...
export default function ComprehensiveUCPs() {
  ...
  const [selectedUcp, setSelectedUcp] = useState<Ucp | null>(null);
  const [isScannerOpen, setIsScannerOpen] = useState(false);
  const [isHistoryOpen, setIsHistoryOpen] = useState(false);
  const [historyUcpId, setHistoryUcpId] = useState<number | null>(null);
  const [isItemsViewerOpen, setIsItemsViewerOpen] = useState(false);
  const [itemsViewerUcpId, setItemsViewerUcpId] = useState<number | null>(null);
  const [itemsViewerUcpCode, setItemsViewerUcpCode] = useState<string>("");
  ...
  const openHistory = (ucp: Ucp) => {
    setHistoryUcpId(ucp.id);
    setIsHistoryOpen(true);
  };

  const openItemsViewer = (ucp: Ucp) => {
    setItemsViewerUcpId(ucp.id);
    setItemsViewerUcpCode(ucp.code);
    setIsItemsViewerOpen(true);
  };

  const openDismantleDialog = (ucp: Ucp) => {
  ...
  return (
    <div className="p-6 space-y-6 animate-fadeInUp">
      ...
      {isLoading ? (
        ...
        <div className={viewMode === "grid" 
          ...
          {filteredUcps.map((ucp, index) => (
            <Card 
              key={ucp.id} 
              className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 group animate-fadeInUp cursor-pointer"
              style={{ animationDelay: `${index * 50}ms` }}
              onClick={() => openItemsViewer(ucp)}
            >
              <CardContent className="p-6">
      ...
      {itemsViewerUcpId && (
        <UcpItemsViewer
          ucpId={itemsViewerUcpId}
          ucpCode={itemsViewerUcpCode}
          isOpen={isItemsViewerOpen}
          onClose={() => {
            setIsItemsViewerOpen(false);
            setItemsViewerUcpId(null);
            setItemsViewerUcpCode("");
          }}
        />
      )}
