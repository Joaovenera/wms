...
interface TruckArrivalWizardProps {
  onPlanCreated?: (planId: number) => void;
}

export function TruckArrivalWizard({ onPlanCreated }: TruckArrivalWizardProps) {
  ...
  // Create truck arrival plan
  const createPlanMutation = useMutation({
    mutationFn: async (data: any) => {
      // Para caminhões, usar placeholder pois geralmente não sabemos qual caminhão chegará
      const vehicleId = await getPlaceholderVehicleId('delivery');
      
      const planData = {
        ...data,
        type: 'truck-arrival-plan',
        status: 'planejamento',
        vehicleId, // Usar veículo placeholder
        notes: `PLANO DE CHEGADA DE CAMINHÃO - ${data.notes || ''}`.trim()
      };
      const res = await apiRequest('POST', '/api/transfer-requests', planData);
      if (!res.ok) {
        const error = await res.json();
        throw new Error(error.message || 'Erro ao criar plano de chegada');
      }
      return await res.json();
    },
    onError: (error) => {
      console.error('Error creating truck arrival plan:', error);
    }
  });
  ...
  const handleCreatePlan = async () => {
    ...
    try {
      // Create the truck arrival plan
      const planData = {
        supplierName,
        estimatedArrival,
        notes,
        // Para planos de chegada de caminhão
  ...
  const wizardProgress = () => {
    let progress = 0;
    if (supplierName) progress += 30;
    if (estimatedArrival) progress += 30;
    if (arrivalItems.length > 0) progress += 40;
    return progress;
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="border-green-200 bg-green-50/30">
        <CardHeader>
          ...
          <CardDescription className="text-green-700">
            Configure o plano de chegada de mercadoria via caminhão
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Dados essenciais sobre a chegada da mercadoria
          </CardDescription>
        </CardHeader>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-yellow-800 text-sm leading-relaxed">
                Os dados específicos do caminhão (placas), informações do motorista e transportadora serão 
                coletados durante a execução na página de <strong>Execução de Carregamento</strong> quando o caminhão chegar.
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                ...
                Itens do Plano de Chegada
              </CardTitle>
              <CardDescription>
                Produtos planejados para chegar no caminhão
              </CardDescription>
            </div>
            <Button
      ...
      <Card>
        ...
        <CardContent>
          <Textarea
            ...
            placeholder="Observações sobre a chegada do caminhão..."
            rows={3}
          />
        </CardContent>
      </Card>
        ...
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Adicionar Item à Chegada de Caminhão
            </DialogTitle>
          </DialogHeader>
